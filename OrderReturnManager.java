...
public class OrderReturnManager {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            System.out.println("7. Count Returns by Status"); 
            ...
            switch (choice) {
                ...
                case 6: searchByOrderId(); break;                   
                case 7: countReturnsByStatus(); break;              
    ...
    private static void searchByOrderId() {
        ...
        try (Connection conn = DBConnection.getConnection1();
            ...
            stmt.setString(1, orderId);
            ResultSet rs = stmt.executeQuery();
            List<ReturnRequest> results = new ArrayList<>();
            while (rs.next()) {
                results.add(new ReturnRequest(
                        rs.getInt("id"),
                        rs.getString("order_id"),
                        rs.getString("customer_name"),
                        rs.getString("reason"),
            ...
            if (results.isEmpty()) {
                System.out.println("No return found with Order ID: " + orderId);
            } else {
    ...
    private static void countReturnsByStatus() {
        String sql = "SELECT status, COUNT(*) AS count FROM return_requests GROUP BY status";
        try (Connection conn = DBConnection.getConnection1();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
            System.out.println("\n=== Return Counts by Status ===");
            while (rs.next()) {
                System.out.printf("%s: %d%n", rs.getString("status"), rs.getInt("count"));
            }
        } catch (SQLException e) {
            System.out.println("Error: " + e.getMessage());
        }
